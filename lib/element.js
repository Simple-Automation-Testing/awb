...
function elementsInitializer(requests, client) {
  ...
  async function callWithWrap(action, ...rest) {
    ...
    if(err && errorType !== retryError) {
      err({sessionId: this.sessionId, selector: this.selector, additionalMessage: driverResp.value, parentSelector: this.baseElement && this.baseElement.selector})
    } else {
      try {
        ...
        const {err, errorType} = handledErrors(driverResp.status)
        if(err) err({sessionId: this.sessionId, selector: this.selector, additionalMessage: driverResp.value, parentSelector: this.baseElement && this.baseElement.selector})
