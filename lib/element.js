...
function elementsInitializer(requests, client) {
  ...
  async function callWithWrap(action, ...rest) {
    ...
    if(err && errorType !== retryError) {
      ...
      try {
        console.log(this.elementId, 'BEFORE ERRORR')
        await this.getTthisElement(true)
        console.log(this.elementId, 'this element id')
        if(typeof rest[0] === 'string') {
          console.log('!!!!!!!!!!! in if')
          rest[0] = this.elementId
        }
        console.log(this.elementId, 'AFTER ERRORR')
        console.log(rest, 'REST OPT')
        driverResp = await action(this.sessionId, ...rest)
        console.log('here', driverResp)
        const {err, errorType} = handledErrors(driverResp.status)
        if(err) {
          err({
            sessionId: this.sessionId,
  ...
  class Element {
    ...
    async isDisplayed(...args) {
      ...
      try {
        await this.getTthisElement(...args)
        console.log(this.elementId, 'IN IS DISPLAYED element layer')
