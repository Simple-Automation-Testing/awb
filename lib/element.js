...
function elementsInitializer(requests, browser) {
  ...
  class Element {
    ...
    constructor(selector, sessionId = browser.sessionId, elementId = null, baseElement = null) {
    ...
    async getSize() {
      ...
      const body = await size(this.sessionId, this.elementId)
    ...
    async getLocation() {
      !this.elementId
        && await this.getTthisElement()
      const body = await location(this.sessionId, this.elementId)
      handledErrors[STATUS_FROM_DRIVER[status]] && handledErrors[STATUS_FROM_DRIVER[status]](this.sessionId, this.selector, JSON.stringify(body.value))
      return body.value
    }

    async getLocationInView() {
      !this.elementId
        && await this.getTthisElement()
      const body = await locationInView(this.sessionId, this.elementId)
      handledErrors[STATUS_FROM_DRIVER[status]] && handledErrors[STATUS_FROM_DRIVER[status]](this.sessionId, this.selector, JSON.stringify(body.value))
      return body.value
    }
  ...
  class Elements {
    ...
    constructor(selector, sessionId = browser.sessionId, baseElement = null) {
      this.baseElement = baseElement
      this.selector = selector
      this.sessionId = sessionId
      this.elements = null
