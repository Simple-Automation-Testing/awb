...
function elementsInitializer(requests, client) {
  ...
  async function elementHandler(error) {
  ...
  class Element {
    ...
    async size() {
      ...
      const driverResp = await size(this.sessionId, this.elementId)
      handledErrors(this, driverResp.status, driverResp.value)
    ...
    async location() {
      !this.elementId
        && await this.getTthisElement()
      const driverResp = await location(this.sessionId, this.elementId)
      handledErrors(this, driverResp.status, driverResp.value)
      return driverResp.value
    }
    ...
    async sendKeys(...keys) {
      !this.elementId
        && await this.getTthisElement()
      const driverResp = await sendKeys(this.sessionId, this.elementId, ...keys)
      handledErrors(this, driverResp.status, driverResp.value)
      return driverResp.value
    }
    ...
    async click() {
      !this.elementId
        && await this.getTthisElement()
      const driverResp = await clickElement(this.sessionId, this.elementId)
      handledErrors(this, driverResp.status, driverResp.value)
      return driverResp.value
    }
    ...
    async isPresent() {
      !this.elementId
        && await this.getTthisElement()
      const driverResp = await present(this.sessionId, this.elementId)
      handledErrors(this, driverResp.status, driverResp.value)
      return driverResp.value
    }
    ...
    async toElement() {
      !this.elementId
        && await this.getTthisElement()
      const driverResp = await executeScript(this.sessionId, 'arguments[0].scrollIntoView()', {
        ELEMENT: this.elementId,
        [WEB_EMENET_ID]: this.elementId
      })
      driverResp
    ...
    async isDisplayed() {
      !this.elementId
        && await this.getTthisElement()
      const driverResp = await displayed(this.sessionId, this.elementId)
      handledErrors(this, driverResp.status, driverResp.value)
      return driverResp.value
    }
