...
function elementsInitializer(requests, client) {
  ...
  async function callWithWrap(action, ...rest) {
    let driverResp = await action(...rest)
    const {err, errorType} = handledErrors(driverResp.status)

    if(err && errorType !== retryError) {
      err({sessionId: this.sessionId, selector: this.selector, additionalMessage: driverResp.value, parentSelector: this.baseElement.selector})
    } else {
      try {
        await this.getTthisElement()
        driverResp = await action(...rest)
        const {err, errorType} = handledErrors(driverResp.status)
        if(err) err({sessionId: this.sessionId, selector: this.selector, additionalMessage: driverResp.value, parentSelector: this.baseElement.selector})
  ...
  class Element {
        ...
        get(target, action) {
          if(action in target) {
            ...
            return async (...args) => {
              ...
              if(waitC.error) throw new InterfaceError(waitEP.error)
    ...
    async size() {
      ...
      return callWithWrap.call(this, size, this.sessionId, this.elementId)
    }

    async location() {
      !this.elementId && await this.getTthisElement()
      return callWithWrap.call(this, location, this.sessionId, this.elementId)
    }

    async locationView() {
      !this.elementId && await this.getTthisElement()
      const driverResp = await locationInView(this.sessionId, this.elementId)
      const {err, errorType} = handledErrors(driverResp.status)
      return driverResp.value
    }
    ...
    async clear() {
      !this.elementId && await this.getTthisElement()
      const driverResp = await clearElementText(this.sessionId, this.elementId)
      const {err, errorType} = handledErrors(driverResp.status)
      return driverResp.value
    }
    ...
    async getTag() {
      !this.elementId && await this.getTthisElement()
      return callWithWrap(this, tagName, this.sessionId, this.elementId)
    }
    ...
    async getTthisElement() {
      ...
      if(this.baseElement) {
        if(!this.baseElement.elementId) {
          ...
          const driverResp = await elementFromElement(this.sessionId, this.baseElement.elementId, this.selector)
          const {err, errorType} = handledErrors(driverResp.status)
          this.elementId = driverResp.value.ELEMENT
        } else {
          const driverResp = await elementFromElement(this.sessionId, this.baseElement.elementId, this.selector)
          const {err, errorType} = handledErrors(driverResp.status)
          this.elementId = driverResp.value.ELEMENT
        ...
        const driverResp = await findElement(this.sessionId, this.selector)
        const {err, errorType} = handledErrors(driverResp.status)
        this.elementId = driverResp.value.ELEMENT
      }
    }
    ...
    async getText() {
      !this.elementId && await this.getTthisElement()
      return callWithWrap.call(this, executeScript, this.sessionId, function() {
        const [element] = arguments
    ...
    async sendKeys(...keys) {
      !this.elementId && await this.getTthisElement()
      return callWithWrap.call(this, sendKeys, this.sessionId, this.elementId, ...keys)
    }

    async getAttribute(attribute) {
      !this.elementId && await this.getTthisElement()
      const driverResp = await getAttribute(this.sessionId, this.elementId, attribute)
      const {err, errorType} = handledErrors(driverResp.status)
      return driverResp.value
    }
    ...
    async click() {
      !this.elementId && await this.getTthisElement()
      return callWithWrap.call(this, clickElement, this.sessionId, this.elementId)
    }

    async isPresent() {
      !this.elementId && await this.getTthisElement()
      return callWithWrap.call(this, present, this.sessionId, this.elementId)
    }

    async toElement() {
      !this.elementId && await this.getTthisElement()
      return callWithWrap.call(this, executeScript, this.sessionId, 'arguments[0].scrollIntoView()', {ELEMENT: this.elementId, [WEB_EMENET_ID]: this.elementId})
    }

    async isDisplayed() {
      !this.elementId && await this.getTthisElement()
      return callWithWrap.call(this, displayed, this.sessionId, this.elementId)
    }
    ...
    async mouseDownAndMove({x, y}) {
      ...
      const driverResp = await moveTo(this.sessionId, {x, y})
      const {err, errorType} = handledErrors(driverResp.status)
      return driverResp.value
    }
  }
  ...
  class Elements {
    ...
    async getElements() {
      ...
      this.elements = []
      if(!this.baseElement) {
        const driverResp = await findElements(this.sessionId, this.selector)
        const {err, errorType} = handledErrors(driverResp.status)
      ...
      if(this.baseElement) {
        ...
        const driverResp = await elementsFromElement(this.sessionId, this.baseElement.elementId, this.selector)
        const {err, errorType} = handledErrors(driverResp.status)
        driverResp.value.forEach(({ELEMENT}) => {
          this.elements.push(new Element(this.selector, this.sessionId, ELEMENT))
        })
