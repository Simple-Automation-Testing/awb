...
function elementsInitializer(requests, client) {
  ...
  class ElementAWB {
    ...
    constructor({selector, sessionId = null, elementId = null, baseElement = null, ...rest}) {
      ...
      // element | elements
      // this.element = initElement.call(this, this, ElementAWB)
      // this.elements = initElement.call(this, this, ElementsAWB)
      // element | elements - alias
      // this.$ = initElement('xxxxx')

      // this.$$ = initElement.call(this, this, ElementsAWB)
    }
    ...
    async waitUntilDisappear(time, index) {
      ...
      try {
        if(this.elementId) {await this.getTthisElement()}
      } catch(error) {
        return
      }
    ...
    async toElement(...args) {
      if(this.subCall) {await this.subCall(); this.subCall = null}
      if(this.elementId) {await this.getTthisElement(args)}
      return callWithWrap.call(this, executeScript, function() {
        const element = arguments[0]
        element.scrollIntoView()
      }, {ELEMENT: this.elementId, [WEB_EMENET_ID]: this.elementId})
