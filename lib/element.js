...
function elementsInitializer(requests, client) {
  ...
  class Element {
    ...
    async waitUntilDisappear(time) {
      await baseNegativeWait(tagName.bind(this, this.sessionId, this.elementId), time)
    }
    ...
    wait(...args) {
      ...
      const thisCall = async (...args) => {
        self.sessionId = self.sessionId || client.sessionId
        if(subCall) {await subCall()}
        const lessTime = await waitEl(self, findElement, present, tagName, time)
        const now = +Date.now()

        do {
          const result = cb.then || returnStringType(cb) === '[object AsyncFunction]' ? await cb(self) : cb(self)
          if(result) return {time: wailtTime - (Date.now() - now)}
          await sleep()
              ...
              return new Proxy({}, {
                get(target, subAction) {
                  return async (...subArgs) => {
                    await thisCall()
                    return await self.elements[index][actionPrev](...args)[subAction](...subArgs)
                  }
                }
              })
            }
