const {returnStringType, baseNegativeWait, sleep, WEB_EMENET_ID, toElementObject} = require('./util')
...
function elementsInitializer(requests, client) {
  ...
  class ElementAWB {
    ...
    async getElementHTML() {
      if(this.subCall) {await this.subCall(); this.subCall = null}
      if(!this.elementId) {await this.getThisElement()}
      return callWithWrap.call(this, executeScript, 'return arguments[0].outerHTML', toElementObject(this.elementId))
    }

    async getColor() {
      if(this.subCall) {await this.subCall(); this.subCall = null}
      if(!this.elementId) {await this.getThisElement()}
      return callWithWrap.call(this, executeScript, 'return window.getComputedStyle(arguments[0]).color', toElementObject(this.elementId))
    }
    ...
    async mouseDownAndMove(...args) {
      ...
      if(!this.elementId) {await this.getThisElement()}
      await callWithWrap.call(this, moveTo, {element: this.elementId})
      await callWithWrap.call(this, buttonDown, toElementObject(this.elementId))
