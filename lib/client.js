...
function initializatorClient(requests, opts) {
  ...
  const {closeApp, getContexts, launchApp, getCurrentActivity, getCurrentPackage, refresh, alertText, alertSetText} = requests
  ...
  class Client {
    ...
    async switchToFrame(element, time = 500) {
      ...
      if(element instanceof ElementAWB && !element.elementId) {
