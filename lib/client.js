...
function initializatorClient(requests, opts) {
  ...
  class Client {
    ...
    async waitForUrlIncludes(url, time) {
      ...
      if(condition.err) return InterfaceError(`Url does not contain ${url} after time ${time} ms`)
    }
    ...
    async waitForTitleInclude(title, time) {
      ...
      if(condition.err) return InterfaceError(`Title does not contain ${title} after time ${time} ms`)
    }

    async resizeWindow(width, height) {
      if(typeof width !== 'number' || typeof height !== 'number') {
        return InterfaceError('Width and height should be a number')
      }
      !this.sessionId && await this.getSession()
    ...
    async wait(...args) {
      ...
      return InterfaceError(message ? message : `Wait condition fail ${time}`)
    }
