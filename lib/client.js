...
const {waitCondition} = require('./util')
...
function initializatorClient(requests, opts) {
  ...
  const {closeApp, getContexts, launchApp, getCurrentActivity, getCurrentPackage, refresh, alertText, alertSetText, moveTo, getScreenshot} = requests
  const {getWindowSize, getCurrentWindowHandle, getCurrentWindowHandles, toFrame, parentFrame, findElement, doubleClick, pressKeys} = requests
  const {executeScript, sleep, backHistory, alertAccept, reset, rotate, shake, lock, alertDismiss, closeCurrentTab, windowSource} = requests
  ...
  class Client {
    ...
    async pressKeys(...keys) {
      return callWithWrap.call(this, pressKeys, keys)
    }

    async pageSource() {
      return callWithWrap.call(this, windowSource)
    }
    ...
    get cookie() {
      return {
        ...
        get: async (key) => {
          return callWithWrap.call(this, cookieG)
        },
        clear: async () => {
          return callWithWrap.call(this, cookieSetOne, key, value)
        },
    ...
    async refresh() {
      return callWithWrap.call(this, refresh)
    }
    ...
    async doubleClick(elemOrPosition) {
      ...
      if(Object.prototype.hasOwnProperty.call(elemOrPosition, 'elementId') && !elemOrPosition.elementId) {
        await elemOrPosition.getTthisElement()
    ...
    async back() {
      return callWithWrap.call(this, backHistory)
    }

    async forward() {
      return callWithWrap.call(this, forwardHistory)
    }

    async takeScreenshot() {
      return callWithWrap.call(this, getScreenshot)
    }

    async saveScreenshot(name, params = {}) {
      !this.sessionId && await this.getSession()
      if(!name) {
        throw new Error(`Name is obligatory to save screenshot`)
      }
      const {
    ...
    async switchToTab(index, wait = null, quantity = 2, title) {
    ...
    async close() {
      if(this.sessionId) {
        return callWithWrap.call(this, killSession)
      }
    }
  }
  return {Client}
...
module.exports = function(opts = defautlOpts) {
  ...
  const elements = initElement(ElementsAWB)
