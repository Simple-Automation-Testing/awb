...
const { waitElementPresent } = require('./util')
...
function initializatorClient(requests, opts) {
  ...
  class Client {
    ...
    async switchToFrame(selector, time = 500) {
      ...
      handledErrors[STATUS_FROM_DRIVER[body.status]] && handledErrors[STATUS_FROM_DRIVER[body.status]](this.sessionId, 'switch to frame', JSON.stringify(body.value))
      return body
    }

    async switchBack() {
      const body = await parentFrame(this.sessionId)
      handledErrors[STATUS_FROM_DRIVER[body.status]] && handledErrors[STATUS_FROM_DRIVER[body.status]](this.sessionId, 'switch back', JSON.stringify(body.value))
    }
    ...
    async closeCurrentTab() {
      ...
      handledErrors[STATUS_FROM_DRIVER[body.status]] && handledErrors[STATUS_FROM_DRIVER[body.status]](this.sessionId, 'close current tab', JSON.stringify(body.value))
      return body
    }
...
module.exports = function (opts = defautlOpts) {
  ...
  return {
      ...
      return new Proxy(null, {
        get(target, action) {
          if (action in Element.prototype) {
            return new Element(...args)[action]
          }
        }
      })
    },
    elements: (...args) => {
      return new Proxy(null, {
        get(target, action) {
          if (action in Elements.prototype) {
            return new Elements(...args)[action]
          }
        }
      })
    },
