...
const handledErrors = (instance, status, additionalMessage) => {
  const {sessionId, parentSelector, selector} = instance
  ...
  if(err) err({sessionId, selector, additionalMessage, parentSelector})
}
