...
async function baseNegativeWait(conditionFn, time, errMessage) {
  ...
  let resp
  while(Date.now() - now < wailtTime) {
    resp = await conditionFn()
    if(resp.status !== 0) return {time: wailtTime - (Date.now() - now)}
    await sleep(callTime)
  }
...
async function baseWait(conditionFn, session, selector, wailtTime) {
  ...
  const errMessage =
    (resp.value.hasOwnProperty('message') && resp.value.message.includes('no such element: Unable to locate elemen')) ?
      resp.value.message : `Element with selector ${JSON.stringify(selector)} does not present`
  return {error: errMessage}
}
