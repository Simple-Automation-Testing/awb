...
async function _fetchy(method, timeout, url, body, opts) {
  ...
  if(contentType && contentType.includes("application/json")) {
    ...
    if(response.status > 300) {
      throw new Error('Something went wrong with request, pleace check driver server connection')
    }
    return {body, headers: response.headers}
  } else {
    if(response.status > 300) {
      throw new Error(`
        Something went wrong with request, pleace check driver server connection
        ${await response.text()}`
      )
    }
