    ...
    return new Promise((resolve) => {
      ...
      const run = () => {
        ...
        if(directConnect && browserName === 'chrome') {
          ...
          args.push(`--port=${self.port}`)
          return spawn(`${GECKO_PATH}`, args)
        } else if(!directConnect && browserName === 'chrome') {
          const args = []
          args.push(`-Dwebdriver.chrome.driver=${CHROME_PATH}`, '-jar', `${STANDALONE_PATH}`)
          return spawn('java', args)
        } else if(!directConnect && browserName === 'firefox') {
          const args = []
          args.push(`-Dwebdriver.gecko.driver=${GECKO_PATH}`, '-jar', `${STANDALONE_PATH}`)
          return spawn('java', args)
        }
      }

      const assertDrivers = (cb, res) => {
        if(directConnect && browserName === 'chrome') {
          if(!fs.existsSync(path.resolve(__dirname, CHROME_PATH))) {
            self.startCb(RUN_PROCCESS.DRIVER_NOT_FOUND)
            return resolve(RUN_PROCCESS.DRIVER_NOT_FOUND)
          }
      ...
      return assertIsFreePort(self.port).then((isFree) => {
        ...
        self.stackString = assertDrivers(self.startCb, resolve)
